...
// if 대통령.대학.StartsWith("서") then "탄핵"
...
let regNum_Int = ref 0

let createIntRegName() = 
  let reg = sprintf"%%i%d" !regNum_Int
  let _ = regNum_Int := !regNum_Int + 1
  reg

let regNum_Bool = ref 0

let createBoolRegName() = 
  let reg = sprintf"%%b%d" !regNum_Bool
  let _ = regNum_Bool := !regNum_Bool + 1
  reg
...
let rec transExp (symtab: SymbolTable) (e: Exp) : Register * Instr list =
  ...
  | Var vname ->
      ...
      let r =
        ...
        | CInt -> createIntRegName()
        | CBool -> createBoolRegName()
        | _ -> createRegName()
      (r, [Load (r, varReg)])
  | Deref vname ->
  ...
  | And (exp1,exp2) ->
    ...
    //   (r4,inst1 @ [GotoIfNot(Reg r1,L1)] @ inst2 @ [Set(r3,Reg r2)] @ [Goto L2] @ [Label L1] @ [Set(r3,Imm 0)] @ [Label L2] @ [Set(r4,Reg r3)] )
  | Or (exp1,exp2) ->
      let (r1,inst1) = transExp symtab exp1
      let (r2,inst2) = transExp symtab exp2      
      let L1 = createLabel()
