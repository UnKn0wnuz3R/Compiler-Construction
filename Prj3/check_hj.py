...
def check(submit_files, point, tc_num, is_delay):
    ...
    for i in range(tc_num):
        ...
        try:
            ...
            if ans.strip() == result:
                cmd = "%s %s %s %s" % (binary, "run-ir", prog_path, input_path)
                output = run_cmd(cmd, timeout=MAX_RUNTIME)
                ir_result, ir_cost = parse_output(output)
                grading_str += "%2d O (OPT: %4d | ORIGIN: %4d)\n" % (i+1,cost, ir_cost)
                obtained_point += point_per_tc * perf_score / 100
            else:
                result = result.replace("\n", " ")
                ans = ans.replace("\n", " ")
                grading_str += " {idx} X (OPT: {result} | ORIGIN: {ans})\n".format(idx=i+1,result=result, ans=ans)
        except subprocess.TimeoutExpired:
            grading_str += "%2d T (0/100) \n" %(i+1)
        except subprocess.CalledProcessError as e:
            grading_str += "%2d E (0/100) \n" %(i+1)
